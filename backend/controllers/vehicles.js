...
async function index(req, res, next) {
  try {
    // get all Vehicles
    res.status(200).json(await Vehicle.find({}).sort("name").populate("start_location","name").populate("end_location","name").populate("capacities","name").populate("capabilities","name").populate("breaks","name"));
  } catch (error) {
    //send error
